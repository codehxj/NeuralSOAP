...
#from funcmol.ceshi import Encoder
...
def save_voxel_eval_nf(config, fabric, occs_pred, occs_gt=None, refine=True, i=0, mols_pred=[], mols_gt=[], mols_pred_dict=defaultdict(list), codes=None, codes_dict=defaultdict(list)):
    dirname_voxels = os.path.join(config["dirname"], "res").replace('\\','/')
...
def save_sdf_eval_nf(config, fabric, mols_pred, mols_gt=None, dec=None, mols_pred_dict=None, codes_dict=None, refine=True):
    ...
    convert_xyzs_to_sdf(mols_pred, os.path.join(dirname_voxels, "pred.sdf"))

    # gt
    if mols_gt is not None:
        convert_xyzs_to_sdf(mols_gt, os.path.join(dirname_voxels, "gt.sdf"))
...
def infer_codes(
    ...
    """
    Infer codes for a given data loader using the provided encoder.
...
def infer_codes_occs_batch(batch, enc, config, to_cpu=False, field_maker=None, code_stats=None, soap_features=None):
    """
    ...
    """
    #batch dict4   'coords'每个原子的三维坐标tensor(2,29,3)  'atoms_channel'每个原子的元素类型整数表示不同的原子类型tensor(2,29)
    # 'radius'每个原子的半径（表示原子大小tensor(2,29)   'xs'在三维空间中为每个分子采样的网格坐标点tensor(2,4000,3)
    #voxels tensor(2,5,16,16,16)
    #occs tensor(2,4000,5)
    #code tensor(2,1024)
    occs, voxels = field_maker.forward(batch)
    # 检查encoder是否支持SOAP特征
    soap_features = batch.get("soap_features", None)  # 从 batch 中获取
    if soap_features is not None and hasattr(enc, 'forward'):
        # 检查encoder的forward方法是否接受soap_features参数

        codes = enc(voxels, soap_features=soap_features)
    else:
        codes = enc(voxels)
