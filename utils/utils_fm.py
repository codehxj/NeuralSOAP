...
#         load_network(checkpoint, model, fabric, is_compile=False, sd="state_dict_ema", net_name="denoiser")
...
def load_checkpoint_fm(
    ...
    if "state_dict_ema" in checkpoint:
        ...
        fabric.print(f">> [DEBUG] Removed 'module.' prefix from {len(old_state_dict)} keys")
    else:
        fabric.print("⚠️ [DEBUG] 'state_dict_ema' not found in checkpoint!")
    # ---------------------------------

    if optimizer is not None:
        load_network(checkpoint, model, fabric, is_compile=False, sd="state_dict_ema", net_name="denoiser")
        optimizer.load_state_dict(checkpoint["optimizer"])
        fabric.print(f">> loaded model and optimizer, trained for {checkpoint['epoch']} epochs")
        return model, optimizer, checkpoint["code_stats"]
    else:
        load_network(checkpoint, model, fabric, is_compile=False, sd="state_dict_ema", net_name="denoiser")
        fabric.print(f">> loaded model trained for {checkpoint['epoch']} epochs")
