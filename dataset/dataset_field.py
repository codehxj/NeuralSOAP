...
class FieldDataset(Dataset):
    ...
    Args:
        ...
        data_small (bool): Use small dataset. Default is False.
        use_soap (bool): Whether to compute SOAP features in real-time. Default is False.
        soap_params (dict): SOAP parameters like {'rcut': 6.0, 'nmax': 8, 'lmax': 6}. Required if use_soap=True.
    """
    def __init__(
        self,
        dset_name: str = "qm9",
        ...
        data_small: bool = False,  # 新增参数
        use_soap: bool = False,    # 是否实时计算 SOAP
        soap_params: dict = None,  # SOAP 参数
        ...
        self.soap_params = soap_params or {'rcut': 6.0, 'nmax': 8, 'lmax': 6}
        self.soap_calculator = None
        if self.use_soap:
            # 初始化 SOAP 计算器
            self.elements_list = ["C", "H", "O", "N", "F"]  # 基于 QM9 硬编码
            self.soap_calculator = SOAP(
                species=self.elements_list,
                r_cut=self.soap_params['rcut'],
                n_max=self.soap_params['nmax'],
                l_max=self.soap_params['lmax'],
                periodic=False
            )
            print(f"实时 SOAP 计算器已初始化: {self.soap_params}")
    ...
    def _read_data(self):
        if self.data_small:
            fname = f"{self.split}_data_small"
        else:
            fname = f"{self.split}_data"
        if self.dset_name == "cremp":  # 修正语法错误
    ...
    def __getitem__(self, index) -> dict:
        ...
        # 实时计算 SOAP 特征（如果启用）
        if self.use_soap and self.soap_calculator is not None:
            soap_feat = self.compute_soap_for_sample(sample, self.soap_calculator,index)
            soap_dim = self.soap_calculator.get_number_of_features()
            if soap_feat is not None:
                sample["soap_features"] = torch.tensor(soap_feat, dtype=torch.float32)
                #print(f"[DEBUG] {self.split} split, index {index}, soap_features shape: {sample['soap_features'].shape}")  # 新增调试
            else:
                # 强制添加 zero，即使失败
                sample["soap_features"] = torch.zeros(soap_dim, dtype=torch.float32)
                print(f"[DEBUG] {self.split} split, index {index}, SOAP compute failed, using zero shape: {sample['soap_features'].shape}")  # 新增调试

        return sample

    def compute_soap_for_sample(self, sample, soap,index):
        """为单个分子样本计算SOAP特征（实时版）"""
        ...
        if not torch.isfinite(coords).all():
            print(f"[DEBUG] {self.split} split, coords has NaN/inf after preprocess: {coords}")
            return None
        ...
        try:
            ...
            return np.mean(soap_desc, axis=0)  # 平均全局 SOAP 向量
        except Exception as e:
            print(f"[DEBUG] {self.split} split, 计算SOAP特征时出错 (index {index}): {e}")
            return None
...
def create_field_loaders(
    ...
    dset = FieldDataset(
        ...
        data_small=config["dset"]["data_small"],
        use_soap=config.get("use_soap", False),  # 新增
        soap_params=config.get("soap_params", None)  # 新增
    )
