...
def show_progress(block_num, block_size, total_size):
    ...
    sys.stdout.write(f'\r[{"{:<50}".format(bar)}] {percent:.2f}%')
    sys.stdout.flush()
...
def download_data(raw_data_dir: str):
    ...
    print("Downloading train_data.pickle...")
    urllib.request.urlretrieve(
        RAW_URL_TRAIN,
        os.path.join(raw_data_dir, "train_data.pickle"),
        reporthook=show_progress
    )
    print("\nDone.")

    print("Downloading val_data.pickle...")
    urllib.request.urlretrieve(
        RAW_URL_VAL,
        os.path.join(raw_data_dir, "val_data.pickle"),
        reporthook=show_progress
    )
    print("\nDone.")

    print("Downloading test_data.pickle...")
    urllib.request.urlretrieve(
        RAW_URL_TEST,
        os.path.join(raw_data_dir, "test_data.pickle"),
        reporthook=show_progress
    )
    print("\nDone.")
