...
class Decoder(nn.Module):
    ...
    def codes_to_molecules(
        ...
        """
        Convert codes to molecular structures with a fixed number of atoms per molecule.
        """
        ...
        for key, value in sorted(mols_dict.items()):
            n_atoms_actual = value[0]["coords"].shape[1] if len(value) > 0 else 0
            fabric.print(f"({n_atoms_actual}, {len(value)})", end=" ")
        fabric.print()
    ...
    def codes_to_grid(
            ...
            codes: torch.Tensor,
            mols_dict: dict = None,
            codes_dict: dict = None,
            config=None,
            fabric=None,
            n_atoms: int = 20,
    ) -> tuple:
        """
        Convert latent codes to preliminary molecular coordinates, limiting each molecule to n_atoms.
        """
        ...
        for idx, grid in tqdm(enumerate(grids)):
            ...
            if mol_init is not None:
                ...
                # 截取前 n_atoms 原子
                mol_init["coords"] = mol_init["coords"][:, :n_atoms]

                num_coords = mol_init["coords"].size(1)
                if num_coords > 0:
                    mols_dict[num_coords].append(mol_init)
                    codes_dict[num_coords].append(codes[idx].cpu())  # 不截 latent code
                else:
                    fabric.print(f"No atoms after truncating n_atoms in molecule {idx}")
            else:
