...
def main(config):
    ...
    nf_checkpoint = torch.load(os.path.join(config["nf_pretrained_path"], "model.pt"), weights_only=False)
...
def train_denoiser(
    ...
    for batch in loader:
        # 提取SOAP特征（如果存在）
        soap_features = batch.get("soap_features", None)
        ...
        if config["on_the_fly"]:
            with torch.no_grad():
                codes, _ = infer_codes_occs_batch(
                    ...
                    soap_features=soap_features  # 传递SOAP特征
                )
        else:
